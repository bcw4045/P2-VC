...
def get_pitch_median(wav, sr: int = None):
    ...
    try:
        # error로 인해 최소 pitch 값 변경 
        # 원래 코드 : 
        pitch = parselmouth.praat.call(sound, "To Pitch", 0.8 / 70, 70, 600)
        #pitch = parselmouth.praat.call(sound, "To Pitch (ac)", 0.8/70, 70, 15, "no", 0.03, 0.45, 0.01, 0.35, 0.14, 600)
        # pitch = parselmouth.praat.call(sound, "To Pitch", 0.8 / 250, 250, 600)
        # pitch = parselmouth.praat.call(
        #     sound, "To Pitch (raw autocorrelation)",
        #     0.8/70,    # Time step
        #     70,        # Pitch floor (70 Hz)
        #     600,       # Pitch ceiling (600 Hz)
        #     15,        # Max. number of candidates
        #     "no",      # Very accurate: "no"로 설정 (혹은 "yes"도 시도 가능하지만 보통 "no"가 낮은 최소 피치를 허용)
    ...
    if pitch_shift_ratio != 1.:
        try:
            ...
            if resultingMinimum < 0:
                new_pitch_median = PRAAT_CHANGEGENDER_PITCHMEDIAN_DEFAULT # 0.0
                pitch_range_ratio = PRAAT_CHANGEGENDER_PITCHRANGERATIO_DEFAULT # 1.0
...
def formant_and_pitch_shift(sound: parselmouth.Sound) -> parselmouth.Sound:
    ...
    """
    formant_shifting_ratio = random.uniform(1.2, 1.5) # ratio > 1, female->fmale
    use_reciprocal = random.uniform(-1, 1) > 0 # ratio < 1, male->female
    if use_reciprocal: # 
        formant_shifting_ratio = 1 / formant_shifting_ratio

    pitch_shift_ratio = random.uniform(1.2, 1.5)
    use_reciprocal = random.uniform(-1, 1) > 0
    if use_reciprocal:
        pitch_shift_ratio = 1 / pitch_shift_ratio

    pitch_range_ratio = random.uniform(1.1, 1.5)
    use_reciprocal = random.uniform(-1, 1) > 0
    if use_reciprocal:
...
def formant_shift(sound: parselmouth.Sound) -> parselmouth.Sound:
    formant_shifting_ratio = random.uniform(1.0, 1.3)
    # formant_shifting_ratio = 1.3
    use_reciprocal = random.uniform(-1, 1) > 0
    if use_reciprocal:
        formant_shifting_ratio = 1 / formant_shifting_ratio
